...
class DbtDefaultTargetsConfig:
    ...
    Target can be specified with:
        - explicit `dmp_af.dbt_target` field in model's config
    ...
    :param default_target: default dbt target for all operators if other not specified explicitly; to set specific
        target for model please use `dmp_af.dbt_target` field in model's config
...
class RetriesConfig:
    """
    Config to parametrize different retry policies for all dmp-af dag components.
...
class Config:
    ...
    historical_pool: Optional[str] = attrs.field(default=None)
