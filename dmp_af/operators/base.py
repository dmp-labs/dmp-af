...
from dmp_af.common.utils import find_latest_log_file, init_environment
from dmp_af.conf import Config, RetryPolicy
...
class DbtBaseOperator(BashOperator):
    ...
    def generate_bash(self, **kwargs) -> str:
        return self._patch_path_to_dbt_bash(
            **kwargs
        ) + f' cd $PATH_TO_DBT  && {self.dmp_af_config.dbt_executable_path} {self.cli} '.format(**kwargs)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_environment = target_environment or dmp_af_config.dbt_default_targets.default_target
        ...
        if self.dmp_af_config.debug_mode_enabled:
        ...
        super().__init__(
            ...
            env=init_environment(self.dmp_af_config) | (env or {}),
            pool=af_pool,
            append_env=True,
            ...
            if self.dmp_af_config.mcd and self.dmp_af_config.mcd.artifacts_export_enabled:
                ...
                try:
                    send_dbt_artifacts_to_montecarlo(
                        ...
                        metastore_name=self.dmp_af_config.mcd.metastore_name,
                        project_name=self.dmp_af_config.dbt_project.dbt_project_name,
                    )
                except Exception as e:
                    logging.warning(f'Failed to send dbt artifacts to MonteCarlo: {e}')
                    if self.dmp_af_config.mcd.success_required:
                        raise e
