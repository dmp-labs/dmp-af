...
from dmp_af.common.constants import DBT_SENSOR_POOL
...
class DbtExternalSensor(ExternalTaskSensor):
    def __init__(
        ...
        **kwargs,
        ...
        retry_policy = dmp_af_config.retries_config.sensor_retry_policy.as_dict()
        ...
        super().__init__(
            ...
            pool=DBT_SENSOR_POOL if dmp_af_config.use_dbt_target_specific_pools else None,
            mode='reschedule',
...
class DbtSourceFreshnessSensor(PythonSensor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_environment = target_environment or dmp_af_config.dbt_default_targets.default_for_tests_target
        self.dmp_af_config = dmp_af_config

        retry_policy = dmp_af_config.retries_config.dbt_source_freshness_retry_policy.as_dict()
    ...
    def _check_freshness(self):
        ...
        with TemporaryDirectory(dir=self.dmp_af_config.dbt_project.dbt_target_path) as tmp_target_path:
            ...
            freshness_cmd = ' && '.join(
                [
                    f'{self.dmp_af_config.dbt_executable_path} source freshness '
                    f'{"--debug" if self.dmp_af_config.debug_mode_enabled else ""} '
                    f'{"-h" if self.dmp_af_config.dry_run else ""} '
            ...
            result = self.subprocess_hook.run_command(
                ...
                env=env,
                cwd=str(self.dmp_af_config.dbt_project.dbt_project_path),
            )
        if result.exit_code:
            return False
