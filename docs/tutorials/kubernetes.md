...
`dmp-af` adds a new profile type that can be used to run tasks in Kubernetes.
...
1. It's required to define `model(dbt, session)` function for dbt to parse the model correctly. There you can define all refs that are used in the model and `dmp-af` will generate these dependencies in the DAG.
