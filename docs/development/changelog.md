...
- New dbt Snapshots (https://docs.getdbt.com/docs/build/snapshots#configuring-snapshots) can be executed by dbtsnapshot task (https://github.com/ijkos/dmp-af/pull/77)
...
- The is_dev flag is deprecated in favor of the dry_run flag in the main
  configuration (https://github.com/ijkos/dmp-af/pull/67)
- Improve the performance of the venv operator (https://github.com/ijkos/dmp-af/pull/65)
- Enhance dbt_run_model with additional CLI options and parameters (https://github.com/ijkos/dmp-af/pull/64)
- Add iceberg compatibility (thanks @authentic7) (https://github.com/ijkos/dmp-af/pull/62)

Fixes:

- Isolates dbt source freshness run in tmp directory (https://github.com/ijkos/dmp-af/pull/67)
- Freeze time for monthly scheduling (https://github.com/ijkos/dmp-af/pull/66)
- Add more flexibility to the running of functional tests (https://github.com/ijkos/dmp-af/pull/65)

Chore:

- add a note section about medium tests configuration (https://github.com/ijkos/dmp-af/pull/67)
...
- add field to the main config to specify custom dbt executable path in the main config for all dbt
  tasks (https://github.com/ijkos/dmp-af/issues/46)

Fixes:

- make less strict dependencies constrains (https://github.com/ijkos/dmp-af/issues/45)
- allow `threads` field to be string in dbt profiles parsing (https://github.com/ijkos/dmp-af/issues/44)
...
- change dbt_run_model DAG name for multi-project usage (https://github.com/ijkos/dmp-af/pull/39)
...
- new [docs page](../configuration/model-config.md) with code references
...
- use a correct path to read `profiles.yml` file (from issue [#19](https://github.com/ijkos/dmp-af/issues/19))
