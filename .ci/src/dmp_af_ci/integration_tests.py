...
class IntegrationTests:
    ...
    async def build_env(
        self,
        source: tp.Annotated[dagger.Directory, DefaultPath('/'), Doc('dmp-af source directory')],
        python_version: str,
    ) -> dagger.Container:
        """
        Prepare python requirements for dmp-af with pinned airflow, python and dbt version
        """
        ...
        base = (
            ...
            # pin python version
            .with_new_file('/dmp_af/.python-version', contents=python_version.base_version)
            .with_workdir('/dmp_af')
        )
